...
#define ASSERT_VALID_SETTABLE_VALUE(value) \
    if (!value) return GEN_ERR;
#define ASSERT_VALID_SETTING_VALUE(value) \
    if (!value) return WRONG_VALUE;
...
bool SortableOIDType::sort_oids(const SortableOIDType *oid1,
                                const SortableOIDType *oid2) {// returns true if oid1 EARLIER than oid2
